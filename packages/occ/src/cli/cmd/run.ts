...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run occ with a message",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      // occ fork: print which config files were loaded. Helps users see whether
      // `occ.jsonc` or `opencode.jsonc` (or neither) is in effect.
      try {
        const fs = await import("node:fs")
        const path = await import("node:path")
        const os = await import("node:os")
        const projectFile = !args.attach
          ? (fs.existsSync("./occ.jsonc") ? "./occ.jsonc"
            : fs.existsSync("./occ.json") ? "./occ.json"
            : fs.existsSync("./opencode.jsonc") ? "./opencode.jsonc"
            : fs.existsSync("./opencode.json") ? "./opencode.json"
            : fs.existsSync("./.occ/config.json") ? "./.occ/config.json"
            : fs.existsSync("./.opencode/config.json") ? "./.opencode/config.json"
            : undefined)
          : undefined
        const globalHome = os.homedir()
        const globalFile =
          fs.existsSync(path.join(globalHome, ".config", "occ", "config.json"))
            ? path.join("~/.config/occ/config.json")
            : fs.existsSync(path.join(globalHome, ".config", "occ", "opencode.json"))
              ? path.join("~/.config/occ/opencode.json")
