...
function globalConfigFile() {
  // occ fork: prefer occ.json(c), then opencode.json(c), then generic config.json
  const candidates = ["occ.jsonc", "occ.json", "opencode.jsonc", "opencode.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      // occ fork: load occ.* first (higher priority), then opencode.* (legacy compat)
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "occ.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "occ.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          // occ fork: accept both `occ.json(c)` and the upstream `opencode.json(c)` names.
          // occ.json(c) takes precedence so users can vendor a project config under the new name.
          for (const file of yield* ConfigPaths.files("occ", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".opencode") || dir.endsWith(".occ") || dir === Flag.OPENCODE_CONFIG_DIR) {
            // occ fork: occ.* first, then opencode.* for legacy
            for (const file of ["occ.json", "occ.jsonc", "opencode.json", "opencode.jsonc"]) {
        ...
        if (existsSync(managedDir)) {
          // occ fork: occ.* first, then opencode.*
          for (const file of ["occ.json", "occ.jsonc", "opencode.json", "opencode.jsonc"]) {
