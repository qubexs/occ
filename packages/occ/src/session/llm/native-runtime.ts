...
import { LLMKeyPool } from "./key-pool"
...
function statusWithFetch(
  ...
  const rotated = LLMKeyPool.pick(providerID)
  const fallback = typeof input.provider.options.apiKey === "string" ? input.provider.options.apiKey : input.provider.key
  const apiKey = rotated ?? fallback
