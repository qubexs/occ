...
export function Autocomplete(props: {
  ...
  const options = createMemo((prev: AutocompleteOption[] | undefined) => {
    ...
    if (!searchValue) {
      // occ fork: pin /models (and its /mo alias) to the top of the empty-input list
      // so users can switch models with one keystroke. Other commands follow.
      const isModelOption = (opt: AutocompleteOption) => {
        const d = (opt.value ?? opt.display).trimEnd()
        return d === "/models" || d === "/mo" || d === "/model"
      }
      const pinned = nonFileOptions.filter(isModelOption)
      const rest = nonFileOptions.filter((o) => !isModelOption(o))
      return [...pinned, ...rest, ...fileOptions]
    }
